...
// BaseObjectType is the type of objects (that is, declared entities)
var BaseObjectType = NewPrimaryKeyType("@object")

// ObjectKind is a case type for distinguishing different kinds of built-in and declared objects
var ObjectKind = NewCaseType(BaseObjectType, "kind")
...
// ErrorTags is a map from error kinds to the corresponding tag
...
// FileType defines a set of constants for supported file types as integers
type FileType int

const (
	_         FileType = iota // it's common in Go to have the first enumerated constant 0 as the default case
	GoFile                    // Indicates that the file is a go code file (with .go extension)
	GoModFile                 // Indicates that the file is a go mod file (with go.mod extension)
)

// UnExtractedType which is of integer type.
type UnExtractedType int

const (
	_          UnExtractedType = iota
	TypeFile                   //UnExtracted File
	TypeFolder                 //UnExtracted Folder
)
