...
/**
 * A high-performance array extension function.
 *
 * Ref: https://dev.to/uilicious/javascript-array-push-is-945x-faster-than-array-concat-1oki
 */
export function extendArray(extendedArray: any[], otherArray: readonly any[]) {
  const extendedArrayLength = extendedArray.length;
  const otherArrayLength = otherArray.length;

  // Pre allocate size
  extendedArray.length = extendedArrayLength + otherArrayLength;

  for (let i = 0; i < otherArrayLength; i++) {
    extendedArray[extendedArrayLength + i] = otherArray[i];
  }
}
